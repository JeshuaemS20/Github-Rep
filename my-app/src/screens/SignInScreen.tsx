...
export default function SignInScreen({ navigation }: Props) {
  ...
  const handleSignIn = async () => {
    ...
    setLoading(true);
    // TODO: Implement actual sign-in logic here (API call)
    setTimeout(() => {
      setLoading(false);
      Alert.alert("Signed in (demo)", "Welcome back!");
      navigation.replace("Calculator");
    }, 1000);
  };

  // FIX: Only pass params accepted by "UserSaveInfo" (equations: string[], userEmail: string)
  const handleSignUp = () => {
    navigation.navigate("UserSaveInfo", { calculations: [], latestDisplay: "0" });
  };

  return (
    <KeyboardAvoidingView
      style={{ flex: 1, backgroundColor: "#15161a" }}
